...
public:
    vector<int> treeQueries(int n, vector<vector<int>>& edges,
        ...
        vector<int> dist(n + 1);
        int time = 0;
        eulerTour(graph, 1, -1, time, inTime, outTime, parent, dist);
        ...
        for (const auto& query : queries) {
            ...
            if (type == 1) {
                ...
                const auto key = make_pair(min(u, v), max(u, v));
                ...
                const int x = query[1];
                res.push_back(tree.query(inTime[x]));
            }
        }
...
private:
    void eulerTour(const vector < vector<pair<int, int>>> & graph, int u,
                   int prev, int& time, vector<int>& inTime,
                   vector<int>& outTime, vector<int>& parent,
                   vector<int>& dist) {
        inTime[u] = time++;
        for (const auto& [v, w] : graph[u]) {
            ...
            dist[v] = dist[u] + w;
            parent[v] = u;
            eulerTour(graph, v, u, time, inTime, outTime, parent, dist);
        }
        outTime[u] = time - 1;
    }
};
